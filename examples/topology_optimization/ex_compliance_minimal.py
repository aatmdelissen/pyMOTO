...
el = E / (12 * (1 + nu) * (1 - 2 * nu)) * np.array([
    ...
    [3 / 2, -ka / 2 - kb / 2, kf, ka / 2 - kb, -kf, -ka + kb / 2, -3 / 2,
     ka + kb]])  # TODO somehow the x and y are mixed up
...
boundary_dofs = np.repeat(domain.get_nodenumber(0, np.arange(ny + 1)) * 2, 2, axis=-1) + np.tile(np.arange(2),
                                                                                                 ny + 1)  # Calculate boundary dof indices
...
sK = func.append(
    pym.AssembleGeneral(sSIMP, domain=domain, element_matrix=el, bc=boundary_dofs))  # Add stiffness assembly module
...
while change > 0.01:
    ...
    print("It {0: 3d}, g0 {1:.3e}, vol {2:.3f}, change {3:.2f}".format(loop, sg0.state, np.sum(sx.state) / (nx * ny),
                                                                       change))
