...
class SolverDenseLU(LinearSolver):
    ...
    def solve(self, rhs, x0=None, trans="N"):
        ...
        if trans == "N":
            ...
            return self.p @ spla.solve_triangular(
                self.l, spla.solve_triangular(self.u, rhs, trans="T"), lower=True, trans="T"
            )
        elif trans == "H":
            return self.p @ spla.solve_triangular(
                self.l, spla.solve_triangular(self.u, rhs, trans="C"), lower=True, trans="C"
            )
        else:
            raise TypeError("Only N, T, and H transposition is possible")
...
class SolverDenseLDL(LinearSolver):
    ...
    def solve(self, rhs, x0=None, trans="N"):
        ...
        if trans == "N":
            ...
            u[self.p] = spla.solve_triangular(
                self.lp, u2, trans="C" if self.hermitian else "T", lower=True, unit_diagonal=True
            )
        elif trans == "T":
