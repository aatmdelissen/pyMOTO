...
than "normal" derivatives. A complex value (:math:`z = x+iy`) can be seen as having two independent variables
(*e.g.*  :math:`x` and :math:`y`, or, :math:`z` and its conjugate :math:`z^*`).

Definition of partial derivatives (Wirtinger derivatives) for the complex value :math:`z = x+iy` are defined as:

  - :math:`\\frac{\partial}{\partial z} = \\frac{1}{2}\left( \\frac{\partial}{\partial x} - i \\frac{\partial}{\partial y}\\right)` (this is stored in :attr:`Signal.sensitivity`)
