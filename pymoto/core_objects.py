...
def fmt_slice(sl):
    ...
    if isinstance(sl, tuple):
        ...
        return (
            f"{'' if sl.start is None else sl.start}:"
    ...
    def add_sensitivity(self, ds: Any):
        ...
        try:
            ...
            if isinstance(ds, type(self.sensitivity)):
                ...
                raise TypeError(
                    f"Adding wrong type '{type(ds).__name__}' to the sensitivity '{type(self.sensitivity).__name__}'"
                    + self._err_str()
                )
        except ValueError:
            ...
            raise ValueError(
                f"Cannot add argument of shape {ds_shape} to the sensitivity of shape {sens_shape}" + self._err_str()
            ) from None
...
class SignalSlice(Signal):
    ...
    def state(self):
        try:
            ...
            raise type(e)(
                str(e) + "\n\t| Above error was raised in SignalSlice.state (getter). Signal details:" + self._err_str()
    ...
    def state(self, new_state):
        try:
            ...
            # Possibilities: Unslicable object (TypeError) or Wrong dimensions or out of range (IndexError)
            raise type(e)(
                str(e) + "\n\t| Above error was raised in SignalSlice.state (setter). Signal details:" + self._err_str()
            ).with_traceback(sys.exc_info()[2])
    ...
    def _wrap_connect(cls, response: BoundMethodT, create=True):
        ...
        def wrapped(self, *args):
            ...
            if all_constant:
                ...
                if create:
                    # Initialize a number of output signals with default names, when not generated by module itself
                ...
                if len(state_out) != self.n_out:
                    raise TypeError("Number of responses calculated ({len(state_out)}) is unequal to number of output "
                                    "signals ({self.n_out})")
    ...
    def response(self):
        ...
        try:
            ...
            raise type(e)(
                ...
                + self._err_str(fn=self.__call__)
            ).with_traceback(sys.exc_info()[2])
    ...
    def sensitivity(self):
        ...
        try:
            ...
            raise type(e)(
                ...
                + self._err_str(fn=self._sensitivity)
            ).with_traceback(sys.exc_info()[2])
...
class Network(Module):
    ...
    def _parse_signal_set(sigs: Any) -> Set[Signal]:
        """Parse signals to a set and unpack sliced signals to their original base signal"""
        def dig_to_base(s: Signal):
            return dig_to_base(s.base) if isinstance(s, SignalSlice) else s
        # Filter out constants and sliced signals to get the orignal
        return set([dig_to_base(s) for s in _parse_to_list(sigs) if _is_valid_signal(s)])
    ...
    def sig_in(self):
        ...
        for m in self.mods:
            all_in.update(self._parse_signal_set(m.sig_in))
            all_out.update(self._parse_signal_set(m.sig_out))
    ...
    def get_input_cone(self, fromsig: SignalsT = None, frommod: ModulesT = None):
        # Set of signals to track forward dependence of (this set grows as the original signals change new signals)
        touched_sig = self._parse_signal_set(fromsig)
        frommod = set(_parse_to_list(frommod))  # Set of modules to track dependency
        ...
        for m in self:
            if m in frommod or any([s in touched_sig for s in self._parse_signal_set(m.sig_in)]):
                # This module is dependent on one or more of the touched signals
                if len(m.sig_out) > 0:
                    # Add the output signals that are dependent to the touched set
                    touched_sig.update(self._parse_signal_set(m.sig_out))
                input_cone.append(m)
        return input_cone

    def get_output_cone(self, tosig: SignalsT = None, tomod: ModulesT = None):
        # Set of signals to track backward dependence
        dependent_sig = self._parse_signal_set(tosig)
        tomod = set(_parse_to_list(tomod))  # Set of modules that are being tracked
        ...
        for m in reversed(self):
            if m in tomod or any([s in dependent_sig for s in self._parse_signal_set(m.sig_out)]):
                # One or more of the dependent signals are affected by this module
                if len(m.sig_in) > 0:
                    # Add the input signals of this module to the dependent set
                    dependent_sig.update(self._parse_signal_set(m.sig_in))
                output_cone.append(m)
        return Network(list(reversed(output_cone)))

    def get_subset(
        self, fromsig: SignalsT = None, tosig: SignalsT = None, include_sinks: bool = True, include_sources: bool = True
    ):
